...
<template>
  <VCard>
    <VDataTable :headers="headers" :items="props.data" :items-per-page="10" :loading="props.loading"
      ...
      <template #item.token="{ item }">
        <div class="d-flex flex-column">
          <span class="text-sm text-success">
            + {{ Number(item.token).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
            ៛
          </span>
        </div>
      </template>
      <!-- Remove <template #bottom /> to show default pagination -->
    </VDataTable>
  </VCard>
</template>
